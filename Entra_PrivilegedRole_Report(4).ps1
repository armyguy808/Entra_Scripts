...
.DESCRIPTION
    ...
    directory role data cannot see:
      ...
      - Defender XDR    : unified RBAC role assignments (Graph beta), with
                          assigned groups expanded to members
      - Windows 365     : Cloud PC RBAC role assignments (Graph beta), with
                          assigned groups expanded to members
...
.PARAMETER SkipDefender
    Skip the Microsoft Defender XDR unified RBAC collection.

.PARAMETER SkipCloudPC
    Skip the Windows 365 (Cloud PC) RBAC collection.
...
.NOTES
    Required Graph API permissions (delegated or application):
      ...
      - RoleManagement.Read.Defender                     (Defender XDR - Part 2, beta)
      - RoleManagement.Read.CloudPC                      (Windows 365 - Part 2, beta)
    ...
    requires an Intune-licensed tenant. Defender XDR and Windows 365 are read
    from Graph BETA endpoints (subject to change); if Defender unified RBAC is
    not active or Cloud PC is unlicensed, those sections warn and continue.
    Use the -Skip* switches to opt out of individual workloads.
#>
...
#   W4. Defender XDR - unified RBAC role assignments via Graph beta
#                                      (RoleManagement.Read.Defender)
#   W5. Windows 365 / Cloud PC - RBAC role assignments via Graph beta
#                                      (RoleManagement.Read.CloudPC)
# ================================================================================
# ================================================================================
...
# Collects role assignments from a Graph unified-RBAC provider (Defender XDR,
# Cloud PC). These providers use unifiedRoleAssignmentMultiple: one assignment
# carries arrays of principalIds, directoryScopeIds, and appScopeIds.
function Add-UnifiedRbacProviderRows {
    param (
        [string]$Workload,
        [string]$ProviderBaseUri
    )

    $defs = Invoke-GraphGetAll -Uri "$ProviderBaseUri/roleDefinitions"
    $defNames = @{}
    foreach ($d in $defs) { $defNames["$($d.id)"] = "$($d.displayName)" }

    $assignments = Invoke-GraphGetAll -Uri "$ProviderBaseUri/roleAssignments"

    foreach ($asg in $assignments) {
        $roleName = $(if ($defNames.ContainsKey("$($asg.roleDefinitionId)")) { $defNames["$($asg.roleDefinitionId)"] } else { "Unknown role ($($asg.roleDefinitionId))" })

        $scopeParts = [System.Collections.Generic.List[string]]::new()
        foreach ($ds in @($asg.directoryScopeIds)) {
            if ("$ds" -eq '/') { $scopeParts.Add('Tenant-wide') }
            elseif ($ds) { $scopeParts.Add("$ds") }
        }
        foreach ($aps in @($asg.appScopeIds)) {
            if ($aps) { $scopeParts.Add("$aps") }
        }
        $scopeDesc = $(if ($scopeParts.Count -gt 0) { $scopeParts -join '; ' } else { 'N/A' })

        foreach ($prinId in @($asg.principalIds)) {
            ...
            $row = @{
                Workload          = $Workload
                Role              = $roleName
                AssignmentName    = $(if ($asg.displayName) { "$($asg.displayName)" } else { 'Role Assignment' })
...
elseif ($exoModulePresent) {
    ...
    try {
        ...
        Write-Warning "Exchange Online collection failed: $_"
    }
    finally {
        if ($exoConnected) {
...
elseif ($exoModulePresent) {
    ...
    try {
        ...
        Write-Warning "Purview collection failed: $_"
    }
    finally {
        if ($ippsConnected) {
            Disconnect-ExchangeOnline -Confirm:$false -ErrorAction SilentlyContinue | Out-Null
...
if ($SkipIntune) {
    ...
    try {
        $intuneRoleDefs = Invoke-GraphGetAll -Uri "https://graph.microsoft.com/v1.0/deviceManagement/roleDefinitions"
        Write-Host "    Processing $($intuneRoleDefs.Count) Intune role definition(s)..." -ForegroundColor Yellow

        foreach ($def in $intuneRoleDefs) {
            $assignments = Invoke-GraphGetAll -Uri "https://graph.microsoft.com/v1.0/deviceManagement/roleDefinitions/$($def.id)/roleAssignments"

            foreach ($summary in $assignments) {
                # The list endpoint returns summaries; fetch the full assignment
                # for members (admin groups) and resourceScopes (scope groups).
                $asg = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/v1.0/deviceManagement/roleDefinitions/$($def.id)/roleAssignments/$($summary.id)" -ErrorAction Stop
...
# --------------------------------------------------------------------------------
if ($SkipDefender) {
    Write-Host "`n[*] Skipping Defender XDR (per -SkipDefender)." -ForegroundColor Gray
}
else {
    Write-Host "`n[*] Collecting Defender XDR role assignments (beta endpoint)..." -ForegroundColor Cyan
    try {
        Add-UnifiedRbacProviderRows -Workload 'Defender XDR' -ProviderBaseUri "https://graph.microsoft.com/beta/roleManagement/defender"
        ...
        Write-Warning "Defender XDR collection failed: $_"
        Write-Warning "Verify RoleManagement.Read.Defender was consented and Defender XDR unified RBAC is active in the tenant."
    }
}
...
# --------------------------------------------------------------------------------
if ($SkipCloudPC) {
    Write-Host "`n[*] Skipping Windows 365 / Cloud PC (per -SkipCloudPC)." -ForegroundColor Gray
}
else {
    Write-Host "`n[*] Collecting Windows 365 (Cloud PC) role assignments (beta endpoint)..." -ForegroundColor Cyan
    try {
        Add-UnifiedRbacProviderRows -Workload 'Windows 365 (Cloud PC)' -ProviderBaseUri "https://graph.microsoft.com/beta/roleManagement/cloudPC"
        ...
        Write-Warning "Windows 365 (Cloud PC) collection failed: $_"
        Write-Warning "Verify RoleManagement.Read.CloudPC was consented and the tenant has Cloud PC licensing."
    }
}
