...
.DESCRIPTION
    ...
    Each part is exported to its own CSV file for review.
...
.PARAMETER WorkloadOutputPath
    Path for the Part 2 (workload roles) CSV. Defaults to current directory
...
.NOTES
    ...
    requires an Intune-licensed tenant. Use -SkipExchangeOnline, -SkipPurview,
    or -SkipIntune to opt out of individual workloads.
#>
...
param (
    ...
    [string]$WorkloadOutputPath = ".\WorkloadRoleReport_$(Get-Date -Format 'yyyyMMdd_HHmmss').csv",
...
# ================================================================================
# PART 2 OUTPUT - Workload role report
# ================================================================================
Write-Host ""
Write-Host "====================================================" -ForegroundColor Green
Write-Host "     PART 2 - WORKLOAD ROLE REPORT SUMMARY" -ForegroundColor Green
Write-Host "====================================================" -ForegroundColor Green

foreach ($wl in @('Exchange Online', 'Purview (Security & Compliance)', 'Intune')) {
    $count = @($workloadReport | Where-Object { $_.Workload -eq $wl }).Count
    Write-Host ("  {0,-32}: {1}" -f $wl, $count)
}
Write-Host ("  {0,-32}: {1}" -f 'Total rows', $workloadReport.Count)
Write-Host ""

if ($workloadReport.Count -eq 0) {
    Write-Warning "No workload role assignments were collected - nothing to export for Part 2. Review the warnings above."
}
else {
    $workloadReport |
        Sort-Object Workload, Role, MemberDisplayName |
        Export-Csv -Path $WorkloadOutputPath -NoTypeInformation -Encoding UTF8

    Write-Host "  Output file: $WorkloadOutputPath" -ForegroundColor Cyan
    Write-Host ""
}
