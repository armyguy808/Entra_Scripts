...
switch ($Cloud) {
    ...
    'GCCHigh' {
        ...
        $ippsAuthUri     = 'https://login.microsoftonline.us/organizations'
    ...
    'DoD' {
        ...
        $ippsConnectionUri = 'https://compliance.dod.microsoft.com/powershell-liveid'
        $ippsAuthUri     = 'https://login.microsoftonline.us/organizations'
        $GraphBaseUri    = 'https://dod-graph.microsoft.us'
    }
}
if ($Cloud -ne 'Commercial') {
    Write-Host "[*] Target cloud: $Cloud (Graph env '$graphEnvName', Azure env '$azEnvName')." -ForegroundColor Magenta
}
...
elseif ($exoModulePresent) {
    ...
    try {
        ...
        if ($ippsConnectionUri) {
            $ippsConnectParams['ConnectionUri'] = $ippsConnectionUri
            $ippsConnectParams['AzureADAuthorizationEndpointUri'] = $ippsAuthUri
        }
...
if (-not $runAzureRbac) {
    ...
    $azModulesPresent = ([bool](Get-Module -ListAvailable -Name Az.Accounts)) -and ([bool](Get-Module -ListAvailable -Name Az.Resources))
    if (-not $azModulesPresent) {
        Write-Warning "Az.Accounts / Az.Resources not found - skipping Azure RBAC. Install with: Install-Module Az.Accounts, Az.Resources -Scope CurrentUser"
    }
    else {
