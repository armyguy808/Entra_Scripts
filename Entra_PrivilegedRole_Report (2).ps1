...
if (-not $runAzureRbac) {
    ...
    if (-not $azModulesPresent) {
        ...
        Write-Host "`n[*] Collecting Azure RBAC subscription role assignments (a separate Azure sign-in may appear)..." -ForegroundColor Cyan
        $azConnected = $false
        try {
            ...
            foreach ($sub in $subscriptions) {
                ...
                foreach ($ra in $azAssignments) {
                    $memberName = $(if ($ra.DisplayName) { "$($ra.DisplayName)" } else { "$($ra.ObjectId)" })
                    $memberId   = $(if ($ra.SignInName) { "$($ra.SignInName)" } else { "$($ra.ObjectId)" })
                    ...
                    if ("$($ra.ObjectType)" -eq 'Group' -and $ra.ObjectId) {
                        $gMembers = @()
                        try {
